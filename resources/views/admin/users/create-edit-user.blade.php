...
@section('breadcrumb')
    <ol class="breadcrumb d-none d-md-block">
        @if(isset($user))
            <li class="breadcrumb-item active" style="display: inline"><a href="{{URL::to('admin/users/update')}}">Editar usuario</a></li>
        @else
            <li class="breadcrumb-item active" style="display: inline"><a href="{{URL::to('admin/users/create')}}">Crear usuario</a></li>
        @endif
        ...
        <div class="col-lg-8 offset-lg-2">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">{{ isset($user) ? Lang::get('Editar usuario') : __('Crear usuario')}}</h4>
                </div>

                <div class="card-body">
                    <form id="update-form" method="POST" action="{{isset($user) ? URL::to('admin/users/update') : URL::to('admin/users/create')}}">
                        @csrf
                                ...
                                <select class="form-control show-tick" id="activity" name="activity" required data-width="100%" data-dropup-auto="false">
                                    ...
                                    @foreach($activities as $activity)
                                        {{--<optgroup label="Grupo {{$activity['group']}}">--}}
                                            <option @if(isset($user) && $user['activity_id'] == $activity['id']) selected="selected" @endif value="{{$activity['id']}}">GRUPO {{$activity['group'].' - '.$activity['name']}}</option>
                                        {{--</optgroup>--}}
                                    @endforeach
                                </select>
